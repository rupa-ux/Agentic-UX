...
export function ShareModal({ open, onClose, onCustomize, themeColor, showSummaryPage, initialTab, hidePreview }: ShareModalProps) {
  ...
  const PageFooter = ({ pageNum, label }: { pageNum?: number; label?: string }) => (
    <div className="flex justify-between items-center px-2.5 py-1 border-t border-[#f0f0f0] mt-auto shrink-0">
      <span className="font-['Inter',sans-serif] text-[4px] text-[#bbb]">Profile performance report</span>
      <span className="font-['Inter',sans-serif] text-[4px] text-[#bbb]">{label || `Page ${pageNum}`}</span>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={`relative flex bg-white rounded-2xl overflow-hidden ${
      ...
      >
        ...
        <div className={`flex flex-col shrink-0 min-h-0 ${hidePreview ? "w-full" : "w-[45%]"}`}>
          ...
          <div className="flex items-center justify-between px-5 pt-4 pb-0 shrink-0">
            <h2
              className="font-['Inter',sans-serif] text-[15px] text-[#111] tracking-[-0.3px]"
            ...
            <button
              onClick={onClose}
              className="w-7 h-7 rounded-lg flex items-center justify-center text-[#aaa] hover:text-[#555] hover:bg-[#f5f5f5] transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-[5px] rounded-lg text-[12px] capitalize transition-all ${
                    isActive
                      ? "bg-[#e8effe] text-[#2552ED]"
                      : "text-[#888] hover:text-[#555] hover:bg-[#f9fafb]"
                  }`}
                  style={{ fontWeight: 400 }}
                >
                  {tab}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
            ...
            {activeTab === "share" && (
              <div className="flex flex-col gap-4">
                {/* Share link */}
                <div>
                  <label className="block text-[11px] text-[#888] mb-1.5" style={{ fontWeight: 400 }}>Share link</label>
                  <div className="flex items-center gap-2 bg-[#f8f9fb] border border-[#eceef2] rounded-lg px-3 h-[36px]">
                    <Link2 className="w-3.5 h-3.5 text-[#b0b0b0] shrink-0" />
                    <span className="flex-1 text-[12px] text-[#2552ED] truncate select-all font-['Inter',sans-serif]">
                    ...
                    <button
                      ...
                      className={`shrink-0 h-[26px] px-2.5 rounded-md text-[11px] flex items-center gap-1 transition-all ${
                        justCopied
                          ...
                          : "bg-white border border-[#e5e7eb] text-[#555] hover:bg-[#f3f4f6] shadow-sm"
                      }`}
                      style={{ fontWeight: 400 }}
                    >
                      {justCopied ? (
                ...
                {/* Who can access */}
                <div>
                  <label className="block text-[11px] text-[#888] mb-1.5" style={{ fontWeight: 400 }}>Who can access?</label>
                  <div className="relative">
                    <button
                      onClick={() => setAccessOpen(!accessOpen)}
                      className="w-full flex items-center justify-between bg-white border border-[#eceef2] rounded-lg px-3 h-[36px] hover:border-[#d0d5dd] transition-colors"
                    >
                      <span className="text-[12px] text-[#333] font-['Inter',sans-serif]">{access}</span>
                      <ChevronDown className={`w-4 h-4 text-[#aaa] transition-transform ${accessOpen ? "rotate-180" : ""}`} />
                    </button>
                    {accessOpen && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#eceef2] rounded-lg shadow-[0_6px_20px_rgba(0,0,0,0.08)] z-30 py-1 overflow-hidden">
                        {["Only invited users", "Anyone with the link", "Public"].map(opt => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 text-[12px] font-['Inter',sans-serif] transition-colors ${
                              access === opt ? "bg-[#e8effe] text-[#2552ED]" : "text-[#333] hover:bg-[#f8f9fb]"
                            }`}
                          >
                            {opt}
                          </button>
                ...
                {/* Invite users */}
                <div>
                  <label className="block text-[11px] text-[#888] mb-1.5" style={{ fontWeight: 400 }}>Invite people</label>
                  <div className="flex items-center bg-white border border-[#eceef2] rounded-lg px-3 h-[36px] focus-within:border-[#7BA3F5] focus-within:shadow-[0_0_0_3px_rgba(37,82,237,0.08)] transition-all">
                    <input
                      type="text"
                      value={inviteSearch}
                      onChange={e => setInviteSearch(e.target.value)}
                      placeholder="Search users or teams..."
                      className="w-full text-[12px] text-[#333] placeholder:text-[#bbb] bg-transparent outline-none font-['Inter',sans-serif]"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                {[
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 px-2.5 py-2.5 rounded-lg hover:bg-[#f8f9fb] transition-colors group"
                  >
                    <div
                    ...
                    <div className="flex-1 text-left">
                      <p className="text-[12px] text-[#333] font-['Inter',sans-serif]" style={{ fontWeight: 400 }}>{f.label}</p>
                      <p className="text-[10px] text-[#aaa] font-['Inter',sans-serif]">{f.sub}</p>
                    </div>
                    <svg className="w-[9px] h-[11px] text-[#d0d0d0] group-hover:text-[#999] transition-colors" viewBox="0 0 10 11.3333" fill="none">
            ...
            {activeTab === "email" && (
              <div className="flex flex-col gap-3.5">
                <div>
                  <label className="block text-[11px] text-[#888] mb-1.5" style={{ fontWeight: 400 }}>Recipients</label>
                  <input
                    type="text"
                    placeholder="Enter email addresses"
                    className="w-full h-[36px] px-3 text-[12px] text-[#333] placeholder:text-[#bbb] border border-[#eceef2] rounded-lg bg-white outline-none font-['Inter',sans-serif] focus:border-[#7BA3F5] focus:shadow-[0_0_0_3px_rgba(37,82,237,0.08)] transition-all"
                  />
                </div>
                <div>
                  <label className="block text-[11px] text-[#888] mb-1.5" style={{ fontWeight: 400 }}>Subject</label>
                  <input
                    type="text"
                    defaultValue="Profile performance report"
                    className="w-full h-[36px] px-3 text-[12px] text-[#333] border border-[#eceef2] rounded-lg bg-white outline-none font-['Inter',sans-serif] focus:border-[#7BA3F5] focus:shadow-[0_0_0_3px_rgba(37,82,237,0.08)] transition-all"
                  />
                </div>
                <div>
                  <label className="block text-[11px] text-[#888] mb-1.5" style={{ fontWeight: 400 }}>Message (optional)</label>
                  <textarea
                    placeholder="Add a message..."
                    rows={3}
                    className="w-full px-3 py-2 text-[12px] text-[#333] placeholder:text-[#bbb] border border-[#eceef2] rounded-lg bg-white outline-none resize-none font-['Inter',sans-serif] focus:border-[#7BA3F5] focus:shadow-[0_0_0_3px_rgba(37,82,237,0.08)] transition-all"
                  />
                </div>
              </div>
            )}
          ...
          {/* Footer bar */}
          <div className={`flex items-center gap-2 px-5 py-3 border-t border-[#f0f0f0] shrink-0 ${hidePreview ? "justify-end" : "justify-between"}`}>
            {!hidePreview && (
              <button
                onClick={onCustomize}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg hover:bg-[#f5f3ff] transition-colors group"
              >
                ...
                <span className="text-[11px] text-[#6d28d9] group-hover:text-[#5b21b6] font-['Inter',sans-serif]" style={{ fontWeight: 400 }}>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={onClose}
                className="px-3 py-[6px] text-[12px] text-[#666] rounded-lg hover:bg-[#f5f5f5] transition-colors font-['Inter',sans-serif]"
                style={{ fontWeight: 400 }}
              >
                Cancel
              </button>
        ...
        {!hidePreview && (
          <div className="w-[55%] bg-[#f2f4f7] border-l border-[#ebebeb] flex flex-col min-h-0 overflow-hidden">

            {/* Preview header */}
            <div className="flex items-center justify-between px-4 h-[38px] shrink-0">
              <span className="text-[10px] text-[#b0b0b0] uppercase tracking-[0.5px] font-['Inter',sans-serif]" style={{ fontWeight: 400 }}>
                Preview
              </span>
              <span className="text-[10px] text-[#ccc] font-['Inter',sans-serif]">
                {totalPages} pages
              </span>
            </div>
